...
def _newton_method(f, Df, x0, epsilon, max_iter):
    ...
    raise NoSolutionException(
        f'Newton Method: no solution found for x0: {x0} - Exceeded maximum iterations {max_iter}')
...
class RTD(object):
    ...
    _eq = sympy.Eq(_zero,
                   _s_r0 * (_D1 + _s_A * _s_t + _s_B * _s_t ** _D2 + _s_C * (
                               _s_t - _D100) * _s_t ** _D3) + _s_rs - _s_rt)
    ...
    def calc_t(self, rt, tolerance=1e-6):
        ...
        if rt < r0:
            ...
            t = _newton_method(f, Df, t, tolerance, 10)
    ...
    def calc_beta(R0, Rtl, Tl, alpha, delta):
        ...
        return (Tl - ((Rtl - R0) / (R0 * alpha) + delta * (Tl / 100 - 1) * (Tl / 100))) / (
                    (Tl / 100 - 1) * (Tl / 100) ** 3)
